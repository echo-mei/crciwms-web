  ...
  <div class="attendanceSummary clearfix">
    <div class="content-box" :class="{'content-style2' : this.treeStyle === 'style2' }">
      ...
      <div class="setting clearfix">
        <div class="left fl">
          <Button size="small" @click="pulldownSummary" class="icon-btn middle-btn">
...
export default {
  data() {
    return {
      ...
      selectCount: 0,
      // 列表配置start
      propMaxHeight: window.innerHeight - 341, //表格的最大高度,默认600,385
      // 表头列数据
      columns: [],
      // 传递参数
      sendData: {
      ...
      listUrl: requestApi.getListSummary, // Table的列表数据请求的地址,385
      treeStyle: "style1"
      // 列表配置end
    };
  },
  ...
  mounted() {
    this.getMonthField();
    window.onresize = () => {
      this.propMaxHeight = window.innerHeight - 341;
    };
    //获取进退场班组,架子队，项目分部
  ...
  methods: {
    ...
    getMonthField() {
      ...
      var curDate = new Date(this.sendData.attenceDate);
